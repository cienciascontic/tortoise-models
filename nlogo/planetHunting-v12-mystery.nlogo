...
;  set yLightList [ ]
  ...
  create-planets 1 [set shape "none"
    set color blue
    set size 2 
...
;    if in-view? u v  [ setxy u v ]
  ...
  create-stars 1 [set shape "none" set color yellow
  ...
  set new-star star 1 
  create-arrowheads 1 [set shape "none" set color red set size 3 st ]
  create-front-planets 1 
    [set shape "none"
    set color blue
    set label "Planet"
    set size 2 ]      ; create a planet and arrowhead that will go behind the sun
  create-front-arrowheads 1  [set shape "none" set color red set size 3 ht ]
...
to handle-messages 
   if custom-planet [
      ask view-displays with [number = 0] [ht set label "Use the mouse to move the planet or use one of the presets." setxy max-pxcor - 25 min-pycor + 3]   
      ask view-displays with [number = 1] [ht set label "When you are ready to run the model, turn off the 'custom-planet' switch." setxy max-pxcor - 20 min-pycor + 1]]
   if not custom-planet  [
      ask view-displays with [number = 0] [ht set label "Drag the mouse up or down to change the tilt." setxy max-pxcor - 30 min-pycor + 3]  
      ask view-displays with [number = 1] [ht set label "To move the planet, turn on the 'custom-planet' switch." setxy max-pxcor - 27 min-pycor + 1]]
end
...
;    set sim-speed slow-speed
    set insert-on? true show "setting insert on to true"
...
;        set heading 180 + towards-nowrap new-planet  
    ...
    ask front-planets [                        ; this follows the new-planet and shows itself only when going in front of the star
      ...
      ifelse  yy >= 0 [ ht ] [if not ([hidden?] of new-planet) [ht]]]                   ; planet is going in back of star
    ask front-arrowheads [                      ; this follows the arrowheads and shows itself only when going in front of the star
      ...
      ifelse  yy >= 0 [ ht ] [if not ([hidden?] of new-planet) [ht]]]                   ; arrowhead is going in back of star
      
    ifelse insert-on? ; support the insert if the insert condition is on
      [ ask insert-star [ht]       ; turn on the inserts
        ask insert-background [ht]
        ask insert-planet [
          ht 
          ...
          ifelse in-insert? u v [ht setxy u v] [ht]
        ...
        if planet-diameter <= 3.4 
          [ ask view-displays with [number = 4] [
              ...
              set label-color red
              ifelse (first [hidden?] of insert-planet) [ask pointers [ hide-link ]] [create-pointers-to insert-planet ask pointers [ hide-link]]
              ]] 
      ]
      [ ask insert-star [ht]           ; turn off inserts
        ...
        set brightness 1]
   ask view-displays with [number = 3] [set label (word "The tilt is " round (90 - theta) " degrees.") ht]
end
...
to handle-presets
     ask planets [ if Preset-planets != "No Planet" [ht]
