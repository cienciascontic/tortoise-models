...
to setup
  clear-all
  ask patches [set no-grass? false]
...
to grow-grass-and-weeds
  ask patches [
    if pcolor = black [
      if random-float 1000 < weed-grow-rate
        [ set pcolor violet
          if grass-suppressed? = true
             [ask neighbors  [set no-grass? true] ]  
           ]
...
to eat-grass  ;; rabbit procedure
  ...
  if pcolor = green  and grass-preference > random 10
  [ set pcolor black
...
to eat-weeds  ;; rabbit procedure
  ...
  if pcolor = violet 
    and weed-preference > random 10 
    and bad-weed-taste < random 10
     [ set pcolor black
       ask patch xcor ycor [ask neighbors [set no-grass? false]]
