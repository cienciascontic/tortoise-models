...
to startup
  ...
  set graph-time-window 700
  set magnify-y-axis 8000
  set telescope-precision 100
  set planet-diameter 37
...
;    if in-view? u v  [ setxy u v ]
  ...
  create-view-displays 1 [set color black set size 0 setxy max-pxcor  max-pycor - 5 set number 3 ] ; used for tilt.
  create-view-displays 1 [set color black set size 0 setxy min-pxcor + 23 max-pycor - insert-size + 1 set number 4 ] ; used for tilt.
  create-view-displays 1 [set color black set size 0 setxy min-pxcor + 30  max-pycor - 2.5 set number 5 ] ; used for planet movement. 
...
to handle-messages 
   ...
   if not custom-planet  [
      ask view-displays with [number = 0] [st set label "Drag the mouse up or down to change the tilt." setxy max-pxcor - 25 min-pycor + 3]  
...
to graph [b v t ]
  ...
  let magnitude precision (v-y-max - (v-y-max - v-y-min) * magnify-y-axis / 9958) 2
  set-plot-y-range (-1 * magnitude) (magnitude)
  plotxy t * days-per-year v 
  ifelse (theta = 0)
   [ ask view-displays with [number = 5] [st set label "Not moving toward or away from you." set label-color yellow setxy 19 ([ycor] of label-turtle + 1)]]
   [ ifelse (v > 0) 
     [ ask view-displays with [number = 5] [st set label "Star moving away from you." set label-color red setxy 15 ([ycor] of label-turtle + 1)]]
     [ ask view-displays with [number = 5] [st set label "Star moving toward you." set label-color cyan setxy 13 ([ycor] of label-turtle + 1)]]
   ]
...
to handle-presets
     ask planets [ if Preset-planets != "No Planet" [st]
        ...
        if Preset-planets = "Earth" [                     ; remember: mass in earth-masses, distances in AU, time in years
          ...
          set planet-diameter 37                           ; the earth is one earth diameter!!
          set rocky-planet true]
...
to new-grid [dia]              ; creates a 10x10 grid of total extent dia. Grid spacing is dia/10
  ...
  let spacing dia / 10         ; Grid spacing is dia/10
    ...
    set label-turtle one-of spots with [xs = 0 and ys = rad]
end
...
251
348
Use the slider above to set how many days of data should be displayed by the graph.
11
52.0
1
