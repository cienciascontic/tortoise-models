...
to startup
  ...
  set slow-speed .5                            ; simulation speed during transit
  ...
  set v-scale .3                               ; a multiplier of v to determine the length of the velocity vector
...
;    if in-view? u v  [ setxy u v ]
  ...
  create-view-displays 1 [set color black set size 0 setxy max-pxcor - 35 min-pycor + 3 set number 0 ht] ; used for instructions. 
  create-view-displays 1 [set color black set size 0 setxy max-pxcor - 33 min-pycor + 1 set number 1 ht] ; used for instructions. 
...
;  set dist distance-to-star                      ; reads the slider that sets the distance to the star. 
  ...
  ask new-planet [ create-links-with arrowheads ]
...
to go                                                    ; this is called from a forever button
  every .01 [ifelse insert-on?                           ; set the simulation speed from the slider normally except during occultation 
      [ set sim-speed  slow-speed ]                      ; slow speed 
...
to handle-messages 
   if custom-planet [
      ask view-displays with [number = 0] [st set label "Use the mouse to move the planet or use one of the presets." setxy max-pxcor - 29 min-pycor + 3]   
      ask view-displays with [number = 1] [st set label "When you are ready to run the model, turn off the 'custom-planet' switch." setxy max-pxcor - 26 min-pycor + 1]]
   if not custom-planet  [
      ask view-displays with [number = 0] [st set label "Move the mouse up or down to change the tilt." setxy max-pxcor - 35 min-pycor + 3]  
      ask view-displays with [number = 1] [st set label "To move the planet, turn on the 'custom-planet' switch." setxy max-pxcor - 33 min-pycor + 1]]
end
...
to scale-variables [d0]
  let d 5
    let f d ^ 1.5
    set bread-crumb-lifetime  .08 *  d0 ^ 1.5            ; the number of years bread-crumbs last
    set time/bread-crumb .0006 *  d0 ^ 1.5              ; the time between bread-crumbs in years
    set v-scale .003 * d0 ^ 1.5
...
to update-screen                                  ; uses dist, and redraws the star, planet, grid, arrowhead and bread-crumbs.  
    ...
    ifelse insert-on? ; support the insert if the insert condition is on
      [ ask insert-star [st]       ; turn on the inserts
        ...
        if planet-diameter <= 3.4 [ask view-displays with [number = 4] [st set label "planet too small to see" set label-color red]]
        ask insert-planet [
          st 
          ...
          let u1 [xcor] of one-of planets
          let u2 [xcor] of one-of stars
...
to-report near-occ?                 ; Tests for occultation. This will result in showing the insert and slowing the simulation
  let u1 [xcor] of one-of planets       ; xcor is the horizontal screen location
  let u2 [xcor] of one-of stars
...
to graph [b v t ]
  ...
  let et precision (t * days-per-year) 0
  ifelse et <  graph-time-window
      [ set xRangeMin 0                             ;this case, time is less than graph-time-window
      set xRangeMax graph-time-window
      ]
      [ set xRangeMin  precision (et - graph-time-window) 2
        set xRangeMax precision (t * days-per-year) 5   ;this case, time is greater than graph-time-window
      ]
  set-current-plot "Velocity of Star"
  set-plot-x-range xRangeMin xRangeMax
  let magnitude precision (v-y-max - (v-y-max - v-y-min) * magnify-y-axis / 9957) 5
...
Preset-Planets
Preset-Planets
"Custom" "Micro planet" "Earth" "Mars" "Jupiter" "Venus" "Like Mercury but 10x larger diameter" "Like Earth but 10x larger diameter" "Earth-like planet near the star" "Large rocky planet very near the star"
2
