...
to startup
  ...
  set old-preset-planets "Planet A"              ; used the first time through to detect user change of the preset-planets selector
  set first-step? true
  ...
  set old-star-type item 1 star-list
...
;  scale-variables grid-scale                       ; certain variables scale with the distance from the obsever to the star   
  ...
  new-grid 2.5
  set Preset-Planets "Planet A - gaseous"                             ; initialize the screen with the earth
  set resetting? false
  handle-presets
;  do-scale-change  
  set theta 0    
  ...
  ask planets [ 
    ifelse resetting? 
      ...
      [ if Preset-planets != "No Planet" [st]
        ...
        if Preset-planets = "Planet A - gaseous" [                     ; remember: mass in earth-masses, distances in AU, time in years
          set x-cor 0.48 / sqrt 2                            ; starts this off 45¡ around the orbit
          set y-cor 0.48 / sqrt 2
          set grid-scale "1 box = 0.25 au"                          ; should be about 5x the orbital radius
          set planet-diameter 33                           ; the earth is one earth diameter!!
          set star-type item 1 star-list
          update-star
          select-circular-velocity                        ; computes vx and vy for a circular orbit at this radius
          set rocky-planet false]
        if Preset-planets = "Planet B - rocky" [                     ; remember: mass in earth-masses, distances in AU, time in years
          set x-cor 1.405102294                            ; starts this off 45¡ around the orbit
          set y-cor 1.558349488
          set vx -3.418628348
          set vy 3.608049466
          set grid-scale "1 box = 0.25 au"                          ; should be about 5x the orbital radius
          set planet-diameter 11                           ; the earth is one earth diameter!!
          set star-type item 3 star-list
          update-star
          set rocky-planet true]
        if Preset-planets = "Planet C - rocky" [                     ; remember: mass in earth-masses, distances in AU, time in years
          set x-cor 0.6766514088784666                            ; starts this off 45¡ around the orbit
          set y-cor 0.7141059377580106
          set vx -4.364466726140016
          set vy 4.43717488714741
          set grid-scale "1 box = 0.25 au"                          ; should be about 5x the orbital radius
          set planet-diameter 16                           ; the earth is one earth diameter!!
          set star-type item 2 star-list
          update-star
          set rocky-planet true]
        if Preset-planets = "Planet D - rocky" [                     ; remember: mass in earth-masses, distances in AU, time in years
          set x-cor 4.5 / sqrt 2                            ; starts this off 45¡ around the orbit
          set y-cor 4.5 / sqrt 2
          set grid-scale "1 box = 1.00 au"                          ; should be about 5x the orbital radius
          set planet-diameter 1                           ; the earth is one earth diameter!!
          set star-type item 4 star-list
          update-star
          select-circular-velocity                        ; computes vx and vy for a circular orbit at this radius
          set rocky-planet true]
...
Preset-Planets
Preset-Planets
"Planet A - gaseous" "Planet B - rocky" "Planet C - rocky" "Planet D - rocky"
0
